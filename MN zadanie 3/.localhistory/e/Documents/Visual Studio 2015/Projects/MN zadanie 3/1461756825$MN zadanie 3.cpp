...
void dane_wykresu(double a, double b, int choice, double tablica[], int *size)
{
	...
	else
	{
		...
		if (choice == 1)
		{
			...
			{
				if (fabs((temp == schemat_hornera(tablica, i, *size))) < EPS) temp = 0;
				danewykresu << i << " " << temp << std::endl;
			}
		}
		...
		else if (choice == 2)
		{
			for (double i = a; i < b; i = i + 0.1)
			{
				if (fabs((temp == 3 * sin(i + 2) + cos(i) - 15 * i) < EPS)) temp = 0;
				danewykresu << i << " " << temp << std::endl;
			}
		}
		else if (choice == 3)
