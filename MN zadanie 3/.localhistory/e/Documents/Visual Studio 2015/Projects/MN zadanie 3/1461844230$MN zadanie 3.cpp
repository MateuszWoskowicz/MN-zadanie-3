...
void dane_wykresu(double a, double b, double roznica, int choice, double tablica[], int*size);
int number_of_lines();
...
int main()
{
	...
	std::cin >> b;
	double roznica;
	roznica = fabs((a - b) / 1000);
	dane_wykresu(a, b, roznica, choice, tablica, &size);
	...
	wczytaj_wezly(z, wezly); // wezly wczytane do wezly[i][j]
...
void dane_wykresu(double a, double b, double roznica, int choice, double tablica[], int*size)
{
	...
	else
	{
		if (choice == 1)
		{
			for (double i = a; i < b; i = i + roznica)
			{
